class Solution:
    def minTimeToReach(self, moveTime: List[List[int]]) -> int:
        ROWS = len(moveTime)
        COLS = len(moveTime[0])
        heap = [(0, 0, 0, 0)] # minHeap of tuples (time, x, y, bit). Start at (0, 0) with time 0 and bit 0
        ...
        while heap:
            time, row, col, bit = heapq.heappop(heap) # Pop the fastest move off the heap
            ...
            if min_time[row][col] == time:
                for dx, dy in directions:
                    ...
                    total_time = max(time, moveTime[new_row][new_col]) + 1 + bit # Add 1 second to account for moving and 0 or 1 depending on the bit's value
                    new_bit = 1 - bit # Flip the bit for the next move
                    ...
                    if total_time < min_time[new_row][new_col]:
                        ...
                        heapq.heappush(heap, (total_time, new_row, new_col, new_bit)) # Push the cell onto the minHeap
